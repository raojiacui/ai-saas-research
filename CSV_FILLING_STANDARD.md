...
虽然标准 CSV 允许“只有字段包含英文逗号、英文双引号或换行时才加引号”，但本仓库的调研表字段很长，常包含 URL、英文关键词、价格、说明文字和从网页复制来的内容。为了让 GitHub、Excel、WPS 和常见预览器都稳定渲染，主表统一使用更保守的写法：

- `products.csv`、`demands.csv`、`backlinks.csv`、`revenue-products.csv` 的所有字段都用英文双引号包起来
- 字段内部出现英文双引号 `"` 时，必须转成两个英文双引号 `""`
- 单元格里不要保留真实换行；把换行改成 ` / `、`；` 或短句连接
- 写入前清理隐藏控制字符，例如从网页复制来的不可见字符
- 每条数据必须占一个物理行，最终文件应为“表头 1 行 + 数据行数”
- 不要在主表中保留空分隔行或“以下是 Agent 自动化找的”这类标记行；需要说明来源时写进已有字段
- 状态类字段不要留空；无法核验时写 `未核验`，不适用时写清楚原因，例如 `不适用（无公开提交入口）`
- 不要为了填满数量编造 AITDK、DR、MRR、价格等数字；查不到就明确写 `未核验` 或保留来源口径

推荐输出形态示例：

```csv
"产品名","网址","给谁用"
"Viggle AI","https://www.viggle.ai/","meme创作者、短视频创作者"
```
...
- 优先用表格软件、脚本或 CSV 导出工具编辑，不要手工拼复杂字段；导出主表时优先选择全字段加引号；导出主表时优先选择全字段加引号
...
- 从网页复制内容时带入了换行或隐藏控制字符，导致一条记录拆成多行或预览器渲染失败
- 空行、备注行或状态字段留空，导致列数不一致或后续筛选判断不清
